...
import { randomBytes as randomBytes$2, pbkdf2Sync, createCipheriv, createDecipheriv, scryptSync, randomUUID, createHmac } from "node:crypto";
...
const GENERAL_CHAT_TEMPLATE = {
  id: "general-chat",
  name: "General Chat",
  description: "Free-form chat with a market-aware assistant. Ask anything, or go deep on a stock, a news topic or your portfolio.",
  icon: "💬",
  defaultIntervalMs: 0,
  defaultSymbols: [],
  ...
  buildPrompt: () => "Open-ended conversation - respond to whatever the user is asking, using tools when real data is needed."
};
...
async function createDefaultDataSources(options = {}) {
...
const POLL_MS = 15e3;
const CONCURRENCY = 4;
let providerConfigsProvider = null;
function setProviderConfigsProvider(fn) {
...
function getMarketDataSources() {
  if (!dataSourcesPromise) {
    dataSourcesPromise = createDefaultDataSources({
      providerConfigs: (providerConfigsProvider == null ? void 0 : providerConfigsProvider()) ?? {}
    });
  }
...
class SkillManager {
  ...
  async testConnection(skillId, values) {
    var _a;
    const skill = BUILTIN_SKILLS.find((s) => s.id === skillId);
    ...
    try {
      const provider = (_a = skill.providerFactory) == null ? void 0 : _a.call(skill, values);
...
async function testBinanceConnection(config) {
  ...
  try {
    ...
    return { ok: true, message: `Authenticated OK (${((_a = body.balances) == null ? void 0 : _a.length) ?? 0} balances visible).` };
  } catch (err) {
    return { ok: false, message: `Connection failed: ${err.message}` };
  }
}
function setupSkillsIpc(manager) {
  ipcMain.handle("skills:list", () => manager.list());
  ipcMain.handle("skills:saveConfig", (_e, args) => {
    return { skills: manager.saveConfig(args.skillId, args.config ?? {}), requiresRestart: true };
  });
  ipcMain.handle("skills:testConnection", (_e, args) => {
    return manager.testConnection(args.skillId, args.config ?? {});
  });
}
function createSkillManager(configDir) {
