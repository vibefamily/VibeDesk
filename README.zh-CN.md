...
> **钱包（账户）+ 数据 + AI = Agent 驱动交易（Wallet (account) + Data + AI = Agentic Trading）**

一款本地优先的桌面应用：你的 AI Agent 读取多源实时行情、分析新闻，并在**获得你的钱包明确授权**后执行真实链上交易。你的钱包、你的 AI、你的数据——除已签名的交易外，一切都不离开你的电脑。**开箱即用**——Agent 运行时内置在应用里，无需自行安装或运行任何服务器。
...
- **进程隔离** — Agent 与钱包运行在独立进程中；Agent 只能通过 IPC **请求签名**，永远无法读取你的密钥 [TODO]
...
- 👛 **本地钱包保险库** — HD 助记词 + 私钥，AES 加密，MetaMask 式交互（查看需输入密码），多账户；私钥永不离主进程
- ⛓️ **链上执行** — 在 Arc 测试网真实执行交易（Uniswap v4 池，gas = 原生 USDC）：报价 → 授权 → 签名 → 广播 → 回执，全部在主进程完成
- 🔄 **Agent 交易闭环** — 可视化完整链路：信号 → 意图 → 报价 → 授权 → 签名 → 广播 → 回执。Agent 提议，人来决策。
- 🌐 **Arc 双网络配置** — 测试网已验证，主网占位就绪（可在设置中切换）
